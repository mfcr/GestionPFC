...
<script> 
  ...
  $(document).ready(function() {
    ...
    $("[id^='btn_clear-']").click(function() {
      ...
      var deleteUrl='/AlumnosBorra/'+currentId;
      var metodo='GET'
    ...
    <div class="row justify-content-center">
        <div class="col-sm-12 mt-2">
            <div class="card bg-secondary sp-2" style='border-width: 5px'>
                ...
                <div class="card-header bg-success text-center" id="headerSaved0" style="display:none;"> <h3><i class="material-icons mr-3" style="font-size:35px;color:white">thumb_up</i>Alumn@ añadido.</h3>   </div>
                ...
                <div name='add_body' id='add_body' class="card-body bg-light" style="display:none;">
                    <form id='form_id-0' name='form_id-0' enctype='multipart/form-data' method='POST' action="/AlumnosCargaIndividual">   
                      @csrf
                      ...
                      <div class='form-row mt-2'>
                        ...
                        <label for="dni" class='col-sm-3'>NIF:</label>
                        <label for="telefono" class='col-sm-3'>Teléfono:</label>
                      ...
                      <div class='form-row'>
                        ...
                        <input type="input" class="form-control col-sm-3" name='dni' id="dni" aria-describedby="dni" placeholder="NIF (8 números y letra sin guiones ni espacios)" form='form_id-0' maxlength="9" >  
                        <input type="input" class="form-control col-sm-3" name='telefono' id="telefono" aria-describedby="telefono" placeholder="Numero de telefono" form='form_id-0' maxlength="15">  
                        <input type="number" id="curso" name="curso" onKeyDown="return false"   class='col-sm-2 text-center'  min="<?php echo($curso);?>"  value="<?php echo($curso);?>" form='form_id-0'>
                      </div>
                      <hr/>
                      <label >Matrículas del alumno para este curso (indique ciclo o deje en blanco):</label>
                      <label >Matrículas del alumno para este curso (indique ciclo o deje en blanco):</label>
                      <div class="form-row ">
                        @for ($i = 0; $i <= 5; $i++)
                          <select  id="ciclo_id-matricula-<?php echo($i);?>" name='ciclo_id-matricula-<?php echo($i);?>' class='form-control col-sm-6' form='form_id-0' >
                  ...
                  <hr/>
                      Notas: <br/> 
                        El dato mínimo para dar de alta un alumn@ es el del email. <br/>
        ...
        <div class="row justify-content-center mt-2"  id="{{'full-'.$it['id']}}"  @if ($it['activo']==false) style="display:none;" @endif >
            <div class="col-sm-12 mt-2">
                <div class="card bg-secondary sp-2" style='border-width: 5px'>
                    ...
                    <div class="card-body bg-light">
                      <form id="{{'form_id-'.$it['id']}}" name="{{'form_id-'.$it['id']}}" method='POST' action="<?php echo($apiUrl); ?>alumnos/<?php echo($it['id']); ?>"> 
                          @csrf
                        <div class='form-row'>
                        ...
                        <div class='form-row mt-2'>
                          ...
                          <label for="dni--<?php echo ($it['id']); ?>" class='col-sm-3'>NIF:</label>
                          <label for="telefono--<?php echo ($it['id']); ?>" class='col-sm-3'>Telefono:</label>
                        ...
                        <div class='form-row'>
                          ...
                          <input type="input" class="form-control col-sm-3" name='dni' id="dni--<?php echo ($it['id']); ?>" aria-describedby="dni" placeholder="NIF (8 números y letra sin guiones ni espacios)" form="{{'form_id-'.$it['id']}}" maxlength="9" value="<?php echo ($it['dni']); ?>" disabled >  
                          <input type="input" class="form-control col-sm-3" name='telefono' id="telefono--<?php echo ($it['id']); ?>" aria-describedby="telefono" placeholder="Numero de telefono" form="{{'form_id-'.$it['id']}}" maxlength="15" value="<?php echo ($it['telefono']); ?>" disabled>
                          <div class="form-group form-inline justify-content-center col-sm-2">
                            ...
                            <label class="form-check-label " for="activo2--<?php echo ($it['id']); ?>">No</label>
                          </div>
                        </div>
                        <hr/><hr/>
                        @if (sizeof($it['proyectos'])>0)
                          <h3 class='text-center'>Proyectos del alumn@:</h3>
                        @else
                          <h3 class='text-center'>No tiene proyectos</h3>
                        @endif

                        @php $borrar=true @endphp

                        <div class="form-group ">
                          @foreach ($it['proyectos'] as $pr)
                            <hr/>
                              @if ($pr['nombreProyecto']==null || $pr['nombreProyecto']=="")
                                <p>{{'Curso: '.$pr['curso'].' - Nombre Proyecto: NOMBRE PROYECTO PENDIENTE   -  Ciclo: '.$pr['ciclos']['codigoCiclo'].'    - Estado: '.$pr['estados']['estado']}}</p>
                              @else
                                <p>{{'Curso: '.$pr['curso'].' - Nombre Proyecto: '.$pr['nombreProyecto'].'   -  Ciclo: '.$pr['ciclos']['codigoCiclo'].'    - Estado: '.$pr['estados']['estado']}}</p>
                              @endif
                            @php if($pr['estados']['id']>1) {$borrar=false;} @endphp
                          @endforeach 
                        </div>
                        <hr/>
                        <div class="form-group text-center">
